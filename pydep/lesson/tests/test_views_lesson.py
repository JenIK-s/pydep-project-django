...
def course(db):
    ...
    module2 = Module.objects.create(title="Test2", description="Test2", image="test/test2.jpg")
...
def test_complete_lesson_marks_done_and_opens_next(client, course, user):
    ...
    url = reverse("lesson:complete_lesson", kwargs={"course_name": course.name, "module_name": module1.title, "lesson_name": lesson1.title})
    response = client.post(url)
    assert response.status_code in (301, 302)
    assert reverse("lesson:module_detail", kwargs={"course_name": course.name, "module_name": module1.title}) in response.url

    p1 = UserLessonProgress.objects.get(user=user, course=course, module=module1, lesson=lesson1)
    assert p1.completed is True
    assert p1.current is False

    p2 = UserLessonProgress.objects.get(user=user, course=course, module=module1, lesson=lesson2)
    assert p2.completed is False
    assert p2.current is True

    url_lesson_1_module_2 = reverse("lesson:lesson_detail", kwargs={"course_name": course.name, "module_name": module2.title, "lesson_name": lesson1.title})
    response = client.get(url_lesson_1_module_2)
    assert response.status_code in (301, 302)

    url = reverse("lesson:complete_lesson", kwargs={"course_name": course.name, "module_name": module1.title, "lesson_name": lesson2.title})
    response = client.post(url)
    assert response.status_code in (301, 302)
    assert reverse("lesson:module_detail", kwargs={"course_name": course.name, "module_name": module1.title}) in response.url
